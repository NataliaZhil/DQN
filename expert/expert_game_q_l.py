...
def main(game_ag, window):
    ...
    while not lost:
        if np.random.random() < game_ag.epsilon:
            game_ag.epsilon = game_ag.epsilon / 1.01 if game_ag.epsilon > 0 else 0
        ...
        if tube.lost(bird.bird_unit()) or g_d.lost(bird.y, bird.rotation):
            reward = -1
            lost = True
            print(state, game_ag.create_q(state), bird.y, tube.y1, bird.bird_unit(), next_state, bird.speed)
        ...
        print(state, game_ag.create_q(state),  final_move)
        game_ag.train_step( state, final_move, reward, next_state, lost, bird.jump_count)
        state = next_state
...
def training(epochs: int = 1000):
    ...
    record = 0
    total_score = 0
    agent_fb = agent.Agent(load_q = True, path = "weights_exper_ql.npy")
    ...
    for epoch in range(epochs):
        ...
        print(
            ...
            Total score {total_score}, Eps{agent_fb.epsilon}, Eps{agent_fb.lr}, {len(agent_fb.Q)}"""
        )
