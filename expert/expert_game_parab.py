...
def main(game_ag, window):
    ...
    max_sc = af.max_score()
    lost = False
    state = state_func(bird.y, tube.y1, tube.y2, tube.x, bird.jump_count)
    reward = 0
    ...
    while not lost:
        ...
        next_state = state_func(bird.y, tube.y1, tube.y2, tube.x, bird.jump_count)
        #print(state, final_move, reward)
        game_ag.remember(state, final_move, reward, next_state, lost)
        game_ag.long_memory()
        state = next_state
...
def training(epochs: int = 1000):
    ...
    for epoch in range(epochs):
        ...
        if not games % 100:
            torch.save(
                agent_fb.model.state_dict(), "weights_exper.h5"
            )
            print("saved")
        total_score += score
        print(
    ...
    print(mean, stdev)
    torch.save(agent_fb.model.state_dict(), "weights_exper.h5")
